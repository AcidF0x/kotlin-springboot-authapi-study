...
class SMSMessageFactory {
    fun get(messageType: SMSMessageType, phoneNumber: String, payload: Map<String, String>): SMSMessageDto {
        ...
        val subjectAndBody = when (messageType) {
            SMSMessageType.SIGN_UP_REQUEST_AUTH_CODE -> Pair("회원가입 인증코드", """
                회원 가입 인증 코드 [${payload["code"]}]
                ${payload["ttl"]}분 안에 입력하세요
            """.trimIndent()
            )
            SMSMessageType.PASSWORD_RESET_REQUEST_AUTH_CODE -> Pair("비밀번호 초기화 인증코드", """
                비밀번호 초기화 인증코드 [${payload["code"]}]
                ${payload["ttl"]}분 안에 입력하세요
            """.trimIndent())
        }
